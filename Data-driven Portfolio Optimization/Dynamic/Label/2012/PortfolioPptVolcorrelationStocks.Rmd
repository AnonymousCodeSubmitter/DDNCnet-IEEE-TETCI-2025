...
price_data1 <- price_data1[,-1]
price_data1 <- na.omit(price_data1)
price_data1 <- price_data1/100 
# Observe data
...
#plot(price_data1$CHNR, type = "l", ylab='', main='Price (HNRG)', cex.main = 2, cex.lab = 1.5, cex.axis = 1.5, lwd=4.0, col = "blue")
```
...
assetsummary<-data.frame(apply(train, 2, mean), apply(train, 2, sd), rhovol, rhosign, apply(train, 2, skewness), apply(train, 2, kurtosis))
#xtable(assetsummary, digits=4)
#assetsummary
```
...
```{r}
p1 <- cbind(min_var1[1:5], 252*min_var1[6], sqrt(252)*min_var1[7], sqrt(252)*min_var1[12])
p2 <- cbind(min_var2[1:5], 252*min_var2[6], sqrt(252)*min_var2[8], sqrt(252)*min_var2[13])
p3 <- cbind(min_var3[1:5], 252*min_var3[6], sqrt(252)*min_var3[9], sqrt(252)*min_var3[14])
p4 <- cbind(min_var4[1:5], 252*min_var4[6], sqrt(252)*min_var4[10],sqrt(252)*min_var4[15])
p5 <- cbind(min_mad[1:5],  252*min_mad[6],  sqrt(252)*min_mad[11],sqrt(252)*min_mad[16])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```

```{r}
p1 <- cbind(max_sr1[1:5], 252*max_sr1[6], sqrt(252)*max_sr1[7], sqrt(252)*max_sr1[12])
p2 <- cbind(max_sr2[1:5], 252*max_sr2[6], sqrt(252)*max_sr2[8], sqrt(252)*max_sr2[13])
p3 <- cbind(max_sr3[1:5], 252*max_sr3[6], sqrt(252)*max_sr3[9], sqrt(252)*max_sr3[14])
p4 <- cbind(max_sr4[1:5], 252*max_sr4[6], sqrt(252)*max_sr4[10],sqrt(252)*max_sr4[15])
p5 <- cbind(max_sr5[1:5], 252*max_sr5[6], sqrt(252)*max_sr5[11],sqrt(252)*max_sr5[16])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```
...
asset_columns <- asset_columns[!is.na(asset_columns) & asset_columns != ""]
